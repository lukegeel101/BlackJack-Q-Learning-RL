...
def compare_parallel_agents(num_hands=1000000):
    """
...
def parallel_train_episodes_optimized(episodes_per_core=8000000):
    """
    Optimized parallel training with better resource utilization
    """
    # Use performance cores only on M2 (typically 4-6 performance cores)
    cores = min(mp.cpu_count() - 2, 6)  # Leave some headroom
    print(f"Using {cores} cores for parallel training")
    print(f"Total episodes: {cores * episodes_per_core:,}")
    
    # Use spawn method for better compatibility with M2
    mp.set_start_method('spawn', force=True)
    
    # Create pool with optimized settings
    with mp.Pool(cores, maxtasksperchild=1) as pool:
        # Create partial function with core IDs
        train_func = partial(single_core_train)
        
        # Use imap for memory efficiency with large episode counts
        chunk_size = max(1, cores // 4)
        results = list(pool.imap(train_func, 
                                 [episodes_per_core] * cores, 
...
def adaptive_episodes_per_core(target_time_minutes=30):
    """
...
def memory_efficient_training(episodes_per_core=8000000, checkpoint_interval=2000000):
    """
...
if __name__ == "__main__":
    ...
    results = parallel_train_episodes_optimized(episodes_per_core=16000000)
    ...
    cProfile.run('single_core_train(10000)', sort='cumtime')
