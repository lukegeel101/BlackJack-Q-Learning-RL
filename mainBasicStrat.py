...
def main():
    """
    Main function to run the basic strategy simulation and display results
    """
    print("Starting Basic Strategy Blackjack Simulation")
    print("(Card counting and Q-learning disabled)")
    
    # Run simulation with 5000 hands
    env, agent, results_df, count_df = simulate_game(
        num_hands=5000, 
        initial_bankroll=10000,
        min_bet=50,    # Flat betting at $50
        max_bet=50     # No bet variation
    )
    
    # Analyze the results
    stats = analyze_simulation(results_df, count_df)
    
    # Compare different flat betting amounts
    print("\n=== Comparing Different Flat Betting Amounts ===")
    
    bet_amounts = [25, 50, 100, 200]
    comparison_results = []
    
    for bet_amount in bet_amounts:
        print(f"Testing ${bet_amount} flat betting...")
        _, _, test_results, _ = simulate_game(
            num_hands=1000, 
            initial_bankroll=10000,
            min_bet=bet_amount,
            max_bet=bet_amount,
        ...
        profit = test_results['reward'].sum()
        house_edge = -profit / (len(test_results) * bet_amount) * 100
        ...
        print(f"  Profit: ${profit:.2f}, House Edge: {house_edge:.3f}%")
    
    # Plot betting comparison
    comparison_df = pd.DataFrame(comparison_results)
    
    plt.figure(figsize=(12, 8))
    
    plt.subplot(2, 1, 1)
    plt.bar(comparison_df['bet_amount'], comparison_df['profit'])
    plt.title('Profit by Bet Amount (1000 hands)')
    plt.xlabel('Flat Bet Amount ($)')
    plt.ylabel('Total Profit ($)')
    plt.grid(axis='y')
    
    plt.subplot(2, 1, 2)
    plt.bar(comparison_df['bet_amount'], comparison_df['house_edge'])
    plt.title('House Edge by Bet Amount')
    plt.xlabel('Flat Bet Amount ($)')
    ...
    plt.tight_layout()
    plt.savefig('betting_amount_comparison.png')
    plt.show()
    
    print("\nBasic Strategy Simulation complete!")
    print("\nKey takeaways:")
    print("- This simulation uses pure basic strategy without any card counting")
    print("- Flat betting is used (no bet variation based on count)")
    print("- Long-term expectation is a small loss due to house edge (~0.5%)")
    print("- Results will vary due to randomness, but should converge to expected values over many hands")
